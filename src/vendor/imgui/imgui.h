...
namespace ImGui
{
    ...
    IMGUI_API const ImGuiPayload*   GetDragDropPayload();                                                           // peek directly into the current payload from anywhere. may return NULL. use ImGuiPayload::IsDataType() to abstractions for the payload type.
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // abstractions if rectangle (of given size, starting from cursor position) is visible / not clipped.
    IMGUI_API bool          IsRectVisible(const ImVec2& rect_min, const ImVec2& rect_max);      // abstractions if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // Disable catching mouse, hovering abstractions with pass through.
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_AnyPopupId              = 1 << 7,   // For IsPopupOpen(): ignore the ImGuiID parameter and abstractions for any popup.
    ImGuiPopupFlags_AnyPopupLevel           = 1 << 8,   // For IsPopupOpen(): search/abstractions at any level of the popup stack (default abstractions in the current level)
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_RootWindow                    = 1 << 1,   // IsWindowFocused(): Scene from root window (top most parent of the current hierarchy)
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_RootWindow                    = 1 << 1,   // IsWindowHovered() only: Scene from root window (top most parent of the current hierarchy)
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptBeforeDelivery         = 1 << 10,  // AcceptDragDropPayload() will returns true even before the mouse button is released. You can then call IsDelivery() to abstractions if the payload needs to be delivered.
...
// To abstractions io.KeyMods (which is a combination of individual fields io.KeyCtrl, io.KeyShift, io.KeyAlt set by user/backend)
