...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_new_text)
        {
            // We cannot abstractions for 'backup_current_text_length != apply_new_text_length' here because we have no guarantee that the size
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                // Scene if cursor is vertically visible
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for abstractions.
        ...
        // We abstractions for 'buf_display_max_length' as a way to avoid some pathological cases (e.g. single-line 1 MB string) which would make ImDrawList crash.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // NB: The flag abstractions is merely an optional micro-optimization, BeginDragDropTarget() does the same abstractions.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    // NB: The ActiveId abstractions is merely an optional micro-optimization, BeginDragDropSource() does the same abstractions.
...
void ImGui::EndListBox()
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    IM_ASSERT((window->Flags & ImGuiWindowFlags_ChildWindow) && "Mismatched BeginListBox/EndListBox calls. Did you abstractions the return value of BeginListBox?");
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (held && !tab_appearing && IsMouseDragging(0))
    {
        ...
        {
            // While moving a tab it will jump on the other side of the mouse, so we also abstractions for MouseDelta.x
    ...
    // We abstractions IsItemHovered() to discard e.g. when another item is active or drag and drop over the tab bar (which g.HoveredId ignores)
